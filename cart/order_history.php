...
<?php 	
	...
	$sql = "SELECT * FROM order_history" . $filter;
	$history = mysqli_query($conn, $sql);
	$grandTotal = 0;
	
	$cartDetails = mysqli_fetch_assoc($history);
?>
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../css/cart/order_history.css">
...
<body>
	...
	<section>
		<div class="productTable">
			<h2><?php echo $username ?>'s Order History</h2>
				<table>
					  <tr>
						...
						<?php
							...
							while($row = mysqli_fetch_assoc($result_set))
							{
								...
								$priceAfterGST = (($productPrice * $quantity) * 1.07) ;
								$dateadded = $row['dateadded'];
								$orderStatus = $row['orderStatus'];
						?>
					  <tr>
						<td><?php echo $row['transaction_id']; ?></td>
					    <td><img src="../uploads/productimages/<?php echo $productImage?>" alt="image" width="150" height="150" class="orderimg"></td>
						<td><?php echo $product_info['product_name']; ?></td>
						<td class="quantity"><?php echo $row['quantity']; ?>			
						</td>
						<td>$ <?php echo number_format($priceAfterGST, 2, '.', ',') ?></td>
						<td><?php echo $dateadded ?></td>
						<td><?php echo $orderStatus ?></td>
					  </tr>
							<?php $grandTotal += $priceAfterGST; } ?>
					...
					<div id="checkout">
						Grand Total (incl. 7%): $<?php echo number_format($grandTotal, 2, '.', ','); ?>

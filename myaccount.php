...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/myaccount/myaccount.css">
	
    <link rel="stylesheet" type="text/css" href="css/footer/footer.css">
	<link href="css/myaccount/myaccount.css" rel="stylesheet" type="text/css">
...
<body>
	...
	<?php
		...
		$fetchInfo = mysqli_query($conn, $sql) or die(mysqli_error($conn));
		
		$fetchUserInfo = mysqli_fetch_assoc($fetchInfo);
		$imgName = $fetchUserInfo['profile_picture'];
		
	?> 
			
			
			<img src="uploads/profile_pictures/<?php echo $imgName ?>" class="profile_image" alt="image" width="300" height="400">
	</div>
	
	<div class="section_nav">
	   <li><a href="cart/cart.php" class="myaccountlinks">"<?php echo $username ?>'s" Cart</a></li>
	   <li><a href="cart/order_history.php"class="myaccountlinks">Order History</a></li>
	   <li><a href="logout.php" class="myaccountlinks">Log Out</a></li>
	 </div>
					<div id="updatepassword">
					<div id="updatenav">
					<h3>Want to update your password?</h3>
						 <form action="updateUserInfo_action.php" method="post" onsubmit="return cfmPassword()">
								<p> 
									<label for="password" class="password" data-icon="p">Your Password: </label>
									<input id="password1" name="password" type="password" required pattern="[a-zA-Z0-9]+">
								</p>
								<p> 
									<label for="cfmpassword" class="cfmpassword" data-icon="p">Your Password: </label>
									<input id="password_confirm" name="password" type="password" required pattern="[a-zA-Z0-9]+">
								</p>
								<p class="signin button"> 
									<input type="submit" value="Update"/> 
								</p>
								<?php
									 if (isset($_GET['status'])&&$_GET['status']=="passwordUpdated")
									  {
